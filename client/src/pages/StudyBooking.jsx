...
const StudyBooking = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-amber-50 font-sans">
      {/* Header */}
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-200 px-6 py-4 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-4">
            <button onClick={() => navigate('/study-support')} className="flex items-center space-x-1 text-slate-500 hover:text-orange-600 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
              <span className="text-sm font-semibold">Study Support</span>
            </button>
            <div className="h-6 w-px bg-slate-200"></div>
            <h1 className="text-xl font-extrabold tracking-tight">
              <span className="text-amber-600">Study Room</span> <span className="text-slate-800">Booking</span>
            </h1>
          </div>
        </div>
      </nav>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border-t border-slate-200 pt-6">
              ...
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-5 mb-5 border border-amber-100">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. John Doe" value={currentStudent.name}
                      ...
                      className="w-full px-3 py-2 rounded-lg border border-amber-200 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 outline-none transition-all text-sm" />
                  </div>
                  <div>
                ...
                <button type="button" onClick={addStudent}
                  className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-all active:scale-95">
            ...
            <div className="flex items-end gap-3">
              <button type="submit" disabled={loading || form.students.length === 0}
                className="flex-1 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg shadow-amber-200 hover:shadow-xl transition-all active:scale-95 disabled:opacity-60">
              ...
              {bookings.filter((b) => b.user?._id === userId).length === 0 ? (
                <div className="text-center py-16">
                  <div className="w-20 h-20 mx-auto mb-4 bg-amber-50 rounded-full flex items-center justify-center">
                    <svg className="w-10 h-10 text-amber-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {bookings.filter((b) => b.user?._id === userId).map((b) => (
                    <div key={b._id} className="group border border-amber-100 rounded-2xl p-5 hover:shadow-lg hover:border-amber-300 transition-all bg-gradient-to-br from-amber-50 to-white">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center space-x-2">
                          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-bold text-sm shadow">
                            {b.roomNumber}
                          </div>
                          <div>
                            <p className="font-bold text-slate-800 text-sm">{b.groupName}</p>
                            <p className="text-xs text-amber-600 font-semibold">Your Booking</p>
                          </div>
                        </div>
                      </div>
                      <div className="space-y-1.5 mb-4">
                        <div className="flex items-center text-sm text-slate-600">
                          <svg className="w-4 h-4 mr-2 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                          {b.date}
                        </div>
                        <div className="flex items-center text-sm text-slate-600">
                          <svg className="w-4 h-4 mr-2 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      ...
                      {b.students && b.students.length > 0 && (
                        <div className="border-t border-amber-200 pt-3 mb-4">
                          ...
                          <div className="space-y-1.5">
                            {b.students.map((student, idx) => (
                              <div key={idx} className="flex items-center text-xs text-slate-700 bg-white rounded px-2 py-1 border border-amber-100">
                                <span className="font-semibold">{student.name}</span>
                                <span className="text-slate-500 ml-1">({student.studentId})</span>
                              </div>
                            ))}
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleEdit(b)} className="flex-1 text-xs font-bold text-amber-700 bg-amber-100 hover:bg-amber-200 py-2.5 rounded-lg transition-all active:scale-95">
