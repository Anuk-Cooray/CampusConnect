...
export default function DashboardSidebar({ userName, studentId = 'IT23328020' }) {
  ...
  return (
    <div className="sticky top-24 space-y-4">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-4 flex items-center gap-4 hover:shadow-md transition-shadow cursor-default">
        <div className="h-14 w-14 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-xl flex-shrink-0 shadow-lg shadow-blue-600/25">
          {userName ? userName.charAt(0).toUpperCase() : 'C'}
        </div>
        <div className="overflow-hidden flex-1 min-w-0">
          <h3 className="font-bold text-slate-900 text-sm truncate" title={userName}>
            {userName}
          </h3>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-2 space-y-0.5">
        {LINKS.map((link) => {
          ...
          return (
            <Link
              key={link.path}
              to={link.path}
              className={`flex items-center gap-3 px-3 py-3 rounded-xl font-semibold text-sm transition-all border-l-4 ${
