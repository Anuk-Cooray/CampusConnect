...
export default function AccommodationCard({ accommodation }: Props) {
  ...
  const BASE = API_BASE;
  const safePrice = Number((accommodation as any).price ?? 0);
  const safeDistance = Number((accommodation as any).distance ?? 0);
  const safeRooms = Number((accommodation as any).availableRooms ?? 0);
  const safeFacilities = Array.isArray((accommodation as any).facilities) ? (accommodation as any).facilities : [];

  const isFull = safeRooms === 0;
      ...
      <div className="relative h-52 overflow-hidden bg-slate-700">
        ...
        <div className="absolute top-3 right-3">
          <span className="text-[11px] font-bold bg-white/90 text-slate-700 px-2.5 py-1 rounded-full shadow">
            {isFull ? "🔴 Full" : `🟢 ${safeRooms} room${safeRooms > 1 ? "s" : ""}`}
          </span>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-slate-500 flex items-center gap-1">
            🎓 {safeDistance} {accommodation.distanceUnit || "km"} from uni
          </span>
